...
type SongRelDbImpl struct {
  db *sql.DB
}

func (c *SongRelDbImpl) GetSongsInAlbum(id string) ([]models.Song, error) {
  ...
  var _id string
  err := c.db.QueryRow("SELECT `id` from `albums` WHERE `id`=?", id).Scan(&_id);
  if err != nil {
  ...
  rows, err := c.db.Query("SELECT `id`, `title`, `genre`, `duration`, `year` FROM `songs` WHERE `albumId`=?", id)
  if err != nil {
    return nil, err
  }
  defer rows.Close()
...
func (c *SongRelDbImpl) CreateOneSongInAlbum(albumId string, song *models.SongDto) (string, error) {

  // Check if albumId exists
  var _albumId string
  err := c.db.QueryRow("SELECT `id` FROM `albums` WHERE `id`=?", albumId).Scan(&_albumId)
  if err != nil {
    if errors.Is(err, sql.ErrNoRows) {
